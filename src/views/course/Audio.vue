  ...
  <div class="content">
    <div class="search">
      ...
      <div class="search_inp">
        <el-form ref="form">
          <el-select v-model="search.status" placeholder="音频状态">
    ...
    >
      ...
      <el-table-column label="音频内容" align="center">
        <template slot-scope="{ row }">
          <div class="context">
...
<script>
import { fetchList, updateAudio, createAudio, deleteAudio } from "@/api/audio";
...
export default {
  ...
  methods: {
    ...
    async handleChangeStatus(row, index) {
      ...
      let mes = await updateAudio({ status, index });
      if (mes.code === 20000) {
        this.$message({
    ...
    async handleDelete(row, index) {
      let status = row.status === 0 ? 1 : 0;
      let mes = await deleteAudio({ status, index });
      if (mes.code === 20000) {
        this.$message({
          message: "删除成功",
          type: "success"
    ...
    handleSubmit() {
      if (this.isEdit) {
        this.$refs.ruleForm.validate(async valid => {
          if (valid) {
            let mes = await updateAudio(this.ruleForm);
            if (mes.code === 20000) {
        ...
        this.$refs.ruleForm.validate(async valid => {
          if (valid) {
            let mes = await createAudio(this.ruleForm);
            if (mes.code === 20000) {
              this.$message.success(mes.data);
              this.isAdd = false;
              this.getaudioList();
